/*!
 * @file PICO_DFRobot_BMX160.cpp
 * @brief define PICO_DFRobot_BMX160 class infrastructure, Modified to use PICO-C-SDK
 * @copyright	Copyright (c) 2010 DFRobot Co.Ltd (http://www.dfrobot.com)
 * @license     The MIT License (MIT)
 * @author Shaqeeb Momen (modification) original author: [luoyufeng] (yufeng.luo@dfrobot.com)
...
DFRobot_BMX160::DFRobot_BMX160(i2c_inst_t *pWire, uint sda, uint scl)
{
    _pWire = pWire;
    _sda = sda;
    _scl = scl;
...
{
    uint8_t *txBuf = (uint8_t *)malloc(sizeof(uint8_t) * (len + 1)); // Allocate a buffer for the data, and internal address
    txBuf[0] = reg;                                                  // Assign first byte as internal address
    memcpy(&txBuf[1], pBuf, len);                                    // Copy rest of the bytes to rest of buffer
    i2c_write_blocking(_pWire, _addr, txBuf, len+1, false);
...
{
    // Write device internal address and send stop condition
    if (i2c_write_blocking(_pWire, _addr, &reg, 1, false) == PICO_ERROR_GENERIC)
        return;
    // Read from device
    i2c_read_blocking(_pWire, _addr, pBuf, len, false);
...
bool DFRobot_BMX160::scan()
{
    int ret;
    uint8_t rxData;
    ret = i2c_read_blocking(_pWire, _addr, &rxData, 1, false);
    //    _pWire->beginTransmission(_addr);
    //     if (_pWire->endTransmission() == 0){
    //         return true;
    //     }
